...
class User(Base):
    ...
    assigned_property_id: Mapped[int | None] = mapped_column(
        ForeignKey("properties.id", ondelete="SET NULL")
    )
    status: Mapped[str] = mapped_column(
        Enum("invited", "verified", "active", "pending_nvm"),
        nullable=False,
        default="invited",
    )
    ...
    bids: Mapped[list["IndexedBid"]] = relationship(
        ...
        foreign_keys="IndexedBid.bidder_wallet",
        primaryjoin="User.wallet_address == IndexedBid.bidder_wallet",
        viewonly=True,
    )
    makelaar_profile: Mapped["Makelaar | None"] = relationship(
        "Makelaar", back_populates="user"
    )
...
class Property(Base):
    ...
    seller: Mapped[User] = relationship(
        "User", back_populates="properties", foreign_keys=[seller_id]
    )
    ...
    auction: Mapped["Auction | None"] = relationship(
        "Auction", back_populates="property", uselist=False
    )
...
class Auction(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    property_id: Mapped[int] = mapped_column(
        ForeignKey("properties.id"), nullable=False, unique=True
    )
...
class IndexedBid(Base):
    ...
    amount_usdc: Mapped[float | None] = mapped_column(
        Numeric(12, 2), nullable=True
    )  # Nooit opslaan — alleen op blockchain
